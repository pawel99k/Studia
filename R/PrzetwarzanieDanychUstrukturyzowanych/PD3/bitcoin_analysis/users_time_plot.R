...
users_time <- read.csv("bitcoin_analysis/users_time.csv")[2:91, 2:3]
...
df <- inner_join(posts_time, BHDI, by = "Date")
...
ggplot(df) + 
  geom_bar(stat = "identity", width = 0.75, aes(Date, Count, fill = "Post Count")) +
  geom_line(aes(Date, Price/4, group = 1, color = "BTC Price")) +
  scale_x_discrete(labels = dates, name = "Date (Month-Year)") +
  scale_y_continuous("Monthly Posts Count",
                     sec.axis = sec_axis(~ . * 4, name = "Monthly BTC Price (BTC/USD)")) +
  labs(title = "Time Series Chart", 
       subtitle = "Monthly Posts Count vs Monthly BTC Price",
       caption = "Monthly BTC Price from https://investing.com/") +
  scale_colour_manual("", values = c("Post Count" = "royalblue", "BTC Price" = "red")) +
